...
/**
 * Send a deal chat message
 */
export async function sendDealChatMessage(data: {
  channelId: string;
  loanId: string;
  content: string;
  ...
  try {
    ...
    if (error) {
      console.error("sendDealChatMessage error:", error);
      return { error: error.message };
    }

    return { success: true, messageId: message?.id };
  } catch (err: unknown) {
    console.error("sendDealChatMessage exception:", err);
    return { error: err instanceof Error ? err.message : "Unknown error" };
  }
}

/**
 * Mark deal chat as read for the current user
 */
export async function markDealChatRead(channelId: string) {
  try {
    const auth = await requireAdmin();
    if ("error" in auth) return { error: auth.error };
    ...
    console.error("markDealChatRead exception:", err);
    return { error: err instanceof Error ? err.message : "Unknown error" };
  }
}

/**
 * Pin/unpin a deal chat message
 */
export async function togglePinDealMessage(
  messageId: string,
  isPinned: boolean
) {
  try {
    ...
    if (error) {
      console.error("togglePinDealMessage error:", error);
      return { error: error.message };
    }

    return { success: true };
  } catch (err: unknown) {
    console.error("togglePinDealMessage exception:", err);
    return { error: err instanceof Error ? err.message : "Unknown error" };
  }
}

/**
 * Get the deal chat channel for a loan, creating one if needed
 */
export async function getOrCreateDealChatChannel(loanId: string) {
  try {
    const auth = await requireAdmin();
    if ("error" in auth) return { error: auth.error };
    ...
    if (existing) return { success: true, channel: existing };

    // Get loan data for channel name
    const { data: loan } = await admin
      .from("loans")
      .select("property_address, type, loan_number")
      .eq("id", loanId)
      .single();

    const channelName = loan
      ? `${loan.property_address ?? "Loan " + (loan.loan_number ?? "")} — ${loan.type ?? "Loan"}`
      : "Deal Chat";
    ...
    if (error) {
      console.error("getOrCreateDealChatChannel error:", error);
      return { error: error.message };
    }

    return { success: true, channel };
  } catch (err: unknown) {
    console.error("getOrCreateDealChatChannel exception:", err);
    return { error: err instanceof Error ? err.message : "Unknown error" };
  }
}
