...
export function Sidebar({ role }: { role: string }) {
  ...
  return (
    <aside
      className={cn(
        "h-screen sticky top-0 border-r bg-navy text-white flex flex-col transition-all duration-300",
        collapsed ? "w-16" : "w-64"
      )}
    >
      <div className="flex items-center justify-between p-4 border-b border-white/10">
        ...
        <button
          ...
          className="p-1 rounded hover:bg-navy-mid/10 transition-colors"
        >
          {collapsed ? (
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive
                  ? "bg-navy-mid/15 text-white"
                  : "text-white/70 hover:bg-navy-mid/10 hover:text-white"
              )}
