-- Create notification trigger for @mentions in the unified notes table.
-- Previously only loan_comments and loan_condition_comments had mention
-- notification triggers. This adds the same capability to the notes table
-- used by CRM contacts, companies, deals, tasks, projects, and approvals.

-- 1. Add a notification type for note mentions
INSERT INTO public.notification_types (
  ...
  'note_mention',
  'Mentioned in a Note',
  'You were @mentioned in a note on a contact, company, deal, or other entity',
  'crm',
  true, true,
  ARRAY['admin', 'super_admin'],
  'normal',
  '{{author_name}} mentioned you in a note',
  'You were mentioned in a note: "{{note_preview}}"',
  true, 22
) ON CONFLICT (slug) DO NOTHING;

-- 2. Trigger function: creates a notification for each mentioned user
...
BEGIN
  ...
  ELSIF NEW.deal_id IS NOT NULL THEN
    ...
    v_action_url := '/admin/pipeline/' || NEW.deal_id::text;
  ...
  ELSIF NEW.opportunity_id IS NOT NULL THEN
    ...
    v_action_url := '/admin/pipeline/' || NEW.opportunity_id::text;
  ...
  ELSIF NEW.condition_id IS NOT NULL THEN
    ...
    v_action_url := '/admin/loans/' || COALESCE(NEW.loan_id::text, '') || '?condition=' || NEW.condition_id::text;

  ELSIF NEW.unified_condition_id IS NOT NULL THEN
    v_entity_type := 'condition';
    v_entity_id := NEW.unified_condition_id;
    v_entity_label := 'Condition';
    v_action_url := '/admin/pipeline/' || COALESCE(NEW.deal_id::text, '') || '?condition=' || NEW.unified_condition_id::text;
...
-- 3. Attach the trigger to the notes table
DROP TRIGGER IF EXISTS on_note_mention_insert ON public.notes;

CREATE TRIGGER on_note_mention_insert
  AFTER INSERT ON public.notes
  FOR EACH ROW
  WHEN (NEW.mentions IS NOT NULL AND array_length(NEW.mentions, 1) > 0)
  EXECUTE FUNCTION public.handle_note_mention_notification();
