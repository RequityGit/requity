...
19. **All relationship fields use `<RelationshipPicker>`.** Import from `@/components/shared/RelationshipPicker`. Never build custom contact/company search UIs. Use `entityType="contact"` or `entityType="company"`. Use `variant="inline"` for hover-to-reveal inline edit fields. The "+ Create" option is always available at the bottom of search results. Search and creation go through `lib/actions/relationship-actions.ts`. Only exceptions: `ContactEmailCombobox` (email recipient selection) and `MultiEmailCombobox` (multi-email badge input).
20. **Use `rq-*` utility classes from `globals.css` for all new UI.** Do not hardcode padding, spacing, or field styles inline. Stat/metric cards must use `.rq-stat-card` class (16px top, 18px sides, 20px bottom). Tab content areas must use `.rq-tab-content` (24px padding). Key-value field sections must use `.rq-field-grid` + `.rq-kv-label` / `.rq-kv-value`. Section headers with action buttons must use `.rq-section-header` pattern. Empty numeric fields display an em-dash (never `0`), empty dates display an em-dash (never format strings). System events in activity feeds use `.rq-system-event`, not the same styling as user content. Refer to `DESIGN_SYSTEM.md` for full token reference before creating any new UI.
