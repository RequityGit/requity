...
export function StreamFilters({ active, counts, onChange }: StreamFiltersProps) {
  return (
    <div className="flex items-center gap-1.5 px-3 py-3 border-b">
      {FILTERS.map(({ key, label }) => (
        <button
          key={key}
          onClick={() => onChange(key)}
          className={cn(
            "flex items-center gap-1 rounded-md border px-2.5 py-1 text-[11px] font-medium rq-transition cursor-pointer",
            active === key
