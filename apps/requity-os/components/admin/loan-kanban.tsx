...
export function LoanKanban({ data, currentUserId }: LoanKanbanProps) {
  ...
  function renderLoanCard(loan: PipelineLoanRow) {
    ...
    return (
      <Card
        ...
        draggable={!isMobile}
        onDragStart={isMobile ? undefined : (e) => handleDragStart(e, loan.id)}
      >
        <CardContent className="p-3">
          <div className="flex items-start gap-1">
            ...
            <div
              ...
              onClick={() => router.push(`/pipeline/${loan.loan_number || loan.id}`)}
            >
              <p className="text-sm font-medium text-foreground truncate">
                {loan.property_address ?? "No address"}
              </p>
              <p className="text-xs text-muted-foreground truncate">{loan.borrower_name}</p>
              <div className="flex items-center justify-between mt-1.5">
                <span className="text-xs font-semibold text-foreground">{formatCurrency(loan.loan_amount)}</span>
                {loan.loan_type && (
                  <span className="text-[10px] text-muted-foreground truncate ml-1">{formatLoanType(loan.loan_type)}</span>
                )}
              </div>
              <div className="flex items-center gap-2 mt-1.5">
                <span className={`inline-flex items-center gap-0.5 text-[10px] font-medium px-1.5 py-0.5 rounded ${getDaysInStageBg(days)}`}>
                  <Clock className="h-2.5 w-2.5" />{days}d
                </span>
                {loan.total_conditions > 0 && (
                  <span className="inline-flex items-center gap-0.5 text-[10px] text-muted-foreground">
                    <FileText className="h-2.5 w-2.5" />{loan.document_count}/{loan.total_conditions}
                  </span>
                )}
              </div>
              {(loan.processor_name || loan.originator_name) && (
                <div className="flex items-center gap-1 mt-1.5">
                  ...
                  <span className="text-[10px] text-muted-foreground truncate">{loan.processor_name || loan.originator_name}</span>
                </div>
              )}
              {loan.next_action && (
                <div className="flex items-start gap-1 mt-1.5 bg-amber-50 rounded px-1.5 py-1">
                  ...
                  <span className="text-[10px] text-amber-700 line-clamp-2">{loan.next_action}</span>
                </div>
              )}
            </div>
          </div>
          <div className="flex justify-between mt-2 pt-2 border-t">
            <Button variant="ghost" size="icon" className="h-8 w-8 md:h-6 md:w-6" disabled={!canMoveLeft || isMoving} onClick={() => moveToStage(loan.id, PIPELINE_STAGES[stageIdx - 1])}>
              <ChevronLeft className="h-3 w-3" />
            </Button>
            <Button variant="ghost" size="icon" className="h-8 w-8 md:h-6 md:w-6" disabled={!canMoveRight || isMoving} onClick={() => moveToStage(loan.id, PIPELINE_STAGES[stageIdx + 1])}>
    ...
    return (
      <div className="space-y-2 pb-4">
        {PIPELINE_STAGES.map((stage) => {
          ...
          const totalVolume = stageLoans.reduce((sum, l) => sum + l.loan_amount, 0);
          return (
            <Collapsible key={stage} defaultOpen={stageLoans.length > 0}>
              <CollapsibleTrigger className="flex items-center justify-between w-full px-3 py-2.5 bg-muted rounded-lg min-h-[44px]">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-semibold text-foreground">{LOAN_STAGE_LABELS[stage]}</h3>
                  <Badge variant="secondary" className="text-xs">{stageLoans.length}</Badge>
                </div>
                {stageLoans.length > 0 && (
              ...
              <CollapsibleContent>
                <div className="space-y-2 pt-2 pl-1 pr-1">
                  {stageLoans.map((loan) => renderLoanCard(loan))}
                </div>
              </CollapsibleContent>
            </Collapsible>
          );
