...
export function BudgetSubTab({
  ...
  if (!budget) {
    return (
      <>
        <EmptyBudgetState onCreateClick={() => setShowCreateModal(true)} />
        <CreateBudgetModal
...
function CreateBudgetModal({
  ...
  function addLineItem() {
    setLineItems((prev) => [
      ...prev,
      { category: "", description: "", budgetAmount: "", notes: "" },
    ]);
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Create Construction Budget</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="General notes about this budget..."
              rows={2}
            />
          </div>
...
function ActiveBudgetView({
  ...
  const totalDrawn = lineItems.reduce((sum, li) => sum + (li.drawn_amount || 0), 0);
  const totalRemaining = totalBudget - totalDrawn;
  const pctDisbursed = totalBudget > 0 ? (totalDrawn / totalBudget) * 100 : 0;
  ...
  const columns: Column<BudgetLineItem>[] = [
    ...
    },
    {
      ...
      header: "% Complete",
      cell: (row) => {
        const revised = row.revised_amount || row.budgeted_amount;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <KpiCard
          title="% Disbursed"
          value={`${pctDisbursed.toFixed(1)}%`}
          icon={<Percent className="h-4 w-4" />}
        />
        <KpiCard
          title="Status"
          value={budget.status}
          icon={<StatusBadge status={budget.status} />}
        />
      </div>
        ...
        <CardContent className="py-3">
          <div className="grid grid-cols-3 md:grid-cols-6 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="font-bold">v{budget.budget_version}</p>
            </div>
            <div>
              <span className="text-xs text-muted-foreground">Line Items</span>
              <p className="font-bold">{activeItems.length}</p>
            </div>
          </div>
        </CardContent>
      </Card>
