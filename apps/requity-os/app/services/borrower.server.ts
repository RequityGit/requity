...
const MEMBER_SELECT =
  "*, contact:crm_contacts!deal_borrower_members_contact_id_fkey(id, first_name, last_name, email, phone)";
...
export async function addBorrowerMember(
  ...
  // If linking to a contact, try to pull defaults from borrower profile
  let profileData: BorrowerProfile | null = null;
  if (member.contact_id) {
    const { data: profile } = await supabase
      ...
      .maybeSingle();
    profileData = profile as BorrowerProfile | null;
  }
  ...
  const insertData = {
    borrowing_entity_id: member.borrowing_entity_id,
    ...
    credit_score: member.credit_score ?? profileData?.default_credit_score ?? 0,
    liquidity: member.liquidity ?? profileData?.default_liquidity ?? 0,
    net_worth: member.net_worth ?? profileData?.default_net_worth ?? 0,
    experience: member.experience ?? profileData?.default_experience ?? 0,
