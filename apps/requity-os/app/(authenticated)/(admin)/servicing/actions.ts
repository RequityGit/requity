...
export async function moveToServicingAction(pipelineLoanId: string) {
  try {
    ...
    // Closing Date is source of truth from pipeline; servicing uses origination_date for the same value.
    const closeDate =
      (loan as { closing_date?: string | null }).closing_date ??
      loan.origination_date ??
      (loan as { funding_date?: string | null }).funding_date;
    if (closeDate) payload.origination_date = closeDate;
