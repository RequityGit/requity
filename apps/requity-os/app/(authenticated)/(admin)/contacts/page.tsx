...
export default async function ContactsPage() {
  ...
  try {
    ...
    teamMembers = (teamResult.data ?? []).map(
    ...
    // Build companies lookup for contact company names and numbers
    const companyLookup: Record<string, { name: string; company_number: string }> = {};
    (companiesResult.data ?? []).forEach((c: { id: string; name: string; company_number: string }) => {
      companyLookup[c.id] = { name: c.name, company_number: c.company_number };
    });

    // Build enriched contact rows
    contactRows = contacts.map((c: any) => ({
      id: c.id,
      ...
      company_name: c.company_id
        ? companyLookup[c.company_id]?.name || c.company_name
        : c.company_name,
      company_id: c.company_id,
      company_number: c.company_id
        ? companyLookup[c.company_id]?.company_number || null
        : null,
    ...
    console.error("CRM contacts page failed to load data:", err);
  }
