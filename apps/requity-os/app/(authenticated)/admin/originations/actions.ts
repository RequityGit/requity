...
// ---------------------------------------------------------------------------
// Deal Chatter — get or create chat channel for an opportunity
// ---------------------------------------------------------------------------

export async function getOrCreateOpportunityChatterChannel(opportunityId: string) {
  try {
    const auth = await requireAdmin();
    if ("error" in auth) return { error: auth.error };
    ...
    // Check if a deal_chat_channel already exists for this opportunity
    const { data: existing } = await (admin as any)
      .from("deal_chat_channels")
      .select("*")
      .eq("loan_id", opportunityId)
      .maybeSingle();

    if (existing) {
      // Also ensure a chat_channels entry exists (for main Chatter feed)
      await ensureChatChannel(admin, opportunityId, existing.channel_name, auth.user.id);
      return { success: true, channel: existing };
    }

    // Fetch opportunity data for channel name
    ...
    const channelName = opp?.deal_name
      ? `${opp.deal_name} — ${opp.loan_type || "Deal"}`
      : "Deal Chatter";

    // Create deal_chat_channel
    const { data: channel, error } = await (admin as any)
      .from("deal_chat_channels")
      .insert({
    ...
    if (error) {
      console.error("getOrCreateOpportunityChatterChannel error:", error);
...
/**
 * Ensure a chat_channels entry of type deal_room exists for this opportunity,
 * so messages show up in the main Chatter page.
 */
async function ensureChatChannel(admin: any, opportunityId: string, name: string, userId: string) {
  try {
    // Check if chat_channels entry already exists
    const { data: existing } = await admin
      .from("chat_channels")
      .select("id")
    ...
    // Create the chat_channels entry
    ...
    if (chatChannel) {
      // Auto-add the creating user as a member
      await admin
    ...
    console.error("ensureChatChannel error:", err);
  }
}
