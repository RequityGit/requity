...
export async function addBorrowerAction(input: AddBorrowerInput) {
  try {
    ...
    const contactNumber = `C-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`;
...
export async function updateBorrowerAction(input: UpdateBorrowerInput) {
  try {
    ...
    revalidatePath(`/admin/borrowers/${input.id}`);
    revalidatePath("/admin/borrowers");
    revalidatePath("/admin/crm");
...
export async function addEntityAction(input: AddEntityInput) {
  try {
    ...
    revalidatePath(`/admin/borrowers/${input.borrower_id}`);
